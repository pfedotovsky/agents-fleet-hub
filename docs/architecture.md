...
| `hooks/useFleet.ts` | The heart of the app: host configs + prefs from storage, 12 s polling loop per host, host status machine, merged cross-host session feed, project creation, star toggle, session rename, login. Successful creates and renames patch the loaded host state without waiting for the next poll. |
