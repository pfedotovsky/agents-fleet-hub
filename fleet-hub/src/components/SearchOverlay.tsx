...
function HighlightedSnippet({ match }: { match: SearchMatch }) {
  ...
  for (const [index, range] of ranges.entries()) {
    ...
    if (end > start) {
      parts.push(
        <mark key={index} className="rounded-sm bg-brass-400/25 px-px text-brass-200">
  ...
  return <span className="line-clamp-2 text-[12px] leading-5 text-ink-500">{parts}</span>
}
  ...
  return (
    ...
    >
      <div
        ...
        onKeyDown={onKeyDown}
        className="flex max-h-[70vh] w-full max-w-2xl flex-col overflow-hidden rounded-xl border border-ink-700/80 bg-ink-900 shadow-2xl"
      >
        <div className="flex items-center gap-2 border-b border-ink-800 px-4 py-3">
          {anySearching ? (
            <LoaderCircle size={15} className="shrink-0 animate-spin text-ink-500" />
          ) : (
          ...
          <button
            type="button"
            onClick={onClose}
            className="shrink-0 rounded p-1 text-ink-600 hover:bg-ink-800 hover:text-ink-300"
          >
            <X size={14} />
          </button>
        </div>
        ...
        <div ref={listRef} className="flex-1 overflow-y-auto p-2">
          {query.trim().length < MIN_QUERY_LENGTH ? (
            <p className="px-3 py-8 text-center text-sm text-ink-600">
              Type at least {MIN_QUERY_LENGTH} characters to search
              {skippedCount > 0 && ` · ${skippedCount} offline host${skippedCount === 1 ? '' : 's'} skipped`}
            </p>
          ) : items.length === 0 && !anySearching ? (
            <p className="px-3 py-8 text-center text-sm text-ink-600">No matches</p>
          ) : (
            items.map((item, index) => {
              ...
              return (
                <div key={item.key}>
                  {showHeader && (
                    <div className="mt-2 flex items-center gap-1.5 px-3 pb-1 pt-1 text-[11px] font-medium text-ink-500 first:mt-0">
                      <span
                  ...
                  <button
                    ...
                    className={`flex w-full flex-col gap-1 rounded-md px-3 py-2 text-left ${
                      index === selected ? 'bg-ink-800' : 'hover:bg-ink-800/50'
                    }`}
                  >
          ...
          {anySearching && (
            <p className="flex items-center justify-center gap-2 px-3 py-3 text-[12px] text-ink-600">
        ...
        <div className="flex items-center gap-3 border-t border-ink-800 px-4 py-2 text-[11px] text-ink-600">
          <span>↑↓ navigate</span>
          <span>↵ open</span>
          <span>esc close</span>
        </div>
